 ...
 * 1. Valida datos
 * 2. Crea profesional + ubicaciones en Supabase usando SERVICE_ROLE (bypassa RLS de forma segura)
 * 3. Manda email al equipo InBody
 * 4. Manda confirmación al doctor
 *
 * Esta función es SERVER-SIDE solamente, la SUPABASE_SERVICE_ROLE_KEY nunca llega al navegador.
 */
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (ubicError) {
      ...
      // Rollback: borrar el profesional
    ...
    try {
      ...
      console.error('Error mandando correos:', emailErr);
      // No fallamos el request si los emails fallan, el registro ya está creado
    }
...
// ============================================================
// PLANTILLAS DE CORREO (mismas del Bloque 3)
// ============================================================
...
<tr><td style="padding:32px;">
  ...
  <div style="background:#fafaf7;border-radius:12px;padding:20px;margin-bottom:24px;">
    ...
    <table width="100%"><tr>
      ${data.foto_perfil_url ? `<td style="width:50%;padding-right:8px;"><img src="${data.foto_perfil_url}" style="width:100%;border-radius:8px;display:block;"/><div style="font-size:11px;color:#5c5c60;text-align:center;margin-top:6px;">Foto de perfil</div></td>` : ''}
